...
const EditProfileModal = ({ profileData, onClose, onSaved }) => {
  ...
  const takePhoto = () => {
    ...
    canvas.toBlob(async (blob) => {
      ...
      try {
        ...
        setForm(prev => ({ 
          ...prev, 
          avatar: uploadRes.data.fileUrl,
          avatarFileId: uploadRes.data.file_id || ''
        }));
  ...
  const handleRemoveAvatar = () => {
    if (window.confirm(lang === 'uz' ? "Profil rasmuni o'chirmoqchimisiz?" : "Remove profile photo?")) {
      setForm(prev => ({ ...prev, avatar: '', avatarFileId: '' }));
...
const Profile = () => {
  ...
  }, [userId, token, user?.id, activeTab]);
  ...
  const displayedPosts = activeTab === 'saved'
    ? savedPosts
    : posts.filter(p => activeTab === 'reels' ? p.isReel || p.fileType === 'video' : !p.isReel);
        ...
        <section className="ip-intro">
          ...
          <div className="ip-stats">
            <div className="ip-stat">
              <b>{formatCount(posts.length)}</b>
              <span>posts</span>
            </div>
            <div className="ip-stat">
              <b>{formatCount(profileData.followers?.length || 0)}</b>
              <span>followers</span>
            </div>
            <div className="ip-stat">
              <b>{formatCount(profileData.following?.length || 0)}</b>
              <span>following</span>
            </div>
          </div>
        </section>
        ...
        <div className="ip-grid">
          {activeTab === 'saved' && savedLoading ? (
            <div className="ip-empty">
              ...
              <p>{lang === 'uz' ? 'Saqlangan postlar yuklanmoqda...' : 'Loading saved posts...'}</p>
            </div>
          ...
          {displayedPosts.length === 0 && (
            <div className="ip-empty">
              {activeTab === 'posts' ? <Grid size={40} opacity={0.3} /> : activeTab === 'reels' ? <Video size={40} opacity={0.3} /> : <Bookmark size={40} opacity={0.3} />}
              <p>{activeTab === 'posts'
                ? t.noPostsYet
                : activeTab === 'reels'
                  ? (lang === 'uz' ? 'Hali reelslar yo\'q' : 'No reels yet')
                  : (lang === 'uz' ? 'Hali saqlangan postlar yo\'q' : 'No saved posts yet')
              }</p>
            </div>
          )}
        </div>
