...
import { 
  Send, ArrowLeft, MoreVertical, Paperclip, Image as ImageIcon, Video, Music, Smile, X, Trash2, UserX,
...
const ChatWindow = ({ friend, messages, onSendMessage, onDeleteMessages, onClearForBoth, onClearForMe, onBack, isOnline, socket }) => {
    ...
    <div className="chat-window">
      <div className="chat-header">
        ...
        {isSelecting ? (
            <div className="selection-actions">
              <button className="delete-btn me" onClick={() => { onDeleteMessages(selectedMessages, false); setIsSelecting(false); setSelectedMessages([]); }}>{t.deleteForMe || 'O\'zimdan'}</button>
              <button className="delete-btn both" onClick={() => { if(window.confirm('Hamma uchun o\'chirilsinmi?')) { onDeleteMessages(selectedMessages, true); setIsSelecting(false); setSelectedMessages([]); } }}>{t.deleteForBoth || 'Hammada'}</button>
              <button className="cancel-selection" onClick={() => { setIsSelecting(false); setSelectedMessages([]); }}><X size={20} /></button>
            </div>
        ) : (
            ...
            <div className={`message-bubble ${msg.isSticker ? 'sticker-msg' : ''} ${msg.isGif ? 'gif-msg' : ''} ${(msg.fileType === 'image' && !msg.text && !msg.isSticker) ? 'image-msg' : ''}`}>
              ...
              {msg.fileType === 'video' && (
                <video
                  ...
                  playsInline
                  className="msg-media"
                  type={msg.fileMimeType || undefined}
                />
              )}
              {msg.fileType === 'audio' && (
        ...
        <div className="preview-float fade-in">
          ...
          <div className="preview-content">
            {preview.type === 'image' ? (
              ...
              <video src={preview.url} controls preload="auto" playsInline />
            ) : (
      ...
      <div className="input-area">
        <form className="input-form" onSubmit={(e) => { e.preventDefault(); uploadAndSend(); }}>
            ...
            <button type="submit" className="send-btn" disabled={uploading || (!text.trim() && !preview)}>
              <Send size={20} />
            </button>
        </form>
      </div>
      ...
      <style jsx="true">{`
        ...
        .delete-btn { padding: 6px 12px; border-radius: 10px; font-size: 0.85rem; font-weight: 700; cursor: pointer; transition: 0.2s; }
        .delete-btn.me { background: rgba(var(--accent-rgb), 0.1); color: var(--accent); }
        .delete-btn.both { background: #ff3b30; color: white; }
        .cancel-selection { color: var(--text-secondary); padding: 4px; }
