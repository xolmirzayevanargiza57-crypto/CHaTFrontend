...
const BottomNavWithAuth = () => {
  const { token } = useAuth();
  const location = useLocation();
  
  // Hide BottomNav on login/auth page or if no token or on stories page
  if (!token || location.pathname === '/login' || location.pathname.startsWith('/stories')) {
    return null;
  }
